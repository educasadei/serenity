...
ErrorOr<void> MainWidget::initialize_menubar(GUI::Window& window)
{
    ...
    m_new_image_action = GUI::Action::create(
        "&New Image...", { Mod_Ctrl, 'N' }, g_icon_bag.filetype_pixelpaint, [&](auto&) {
    ...
    m_new_image_from_clipboard_action = GUI::Action::create(
        "&New Image from Clipboard", { Mod_Ctrl | Mod_Shift, 'V' }, g_icon_bag.new_clipboard, [&](auto&) {
    ...
    m_close_image_action = GUI::Action::create("&Close Image", { Mod_Ctrl, 'W' }, g_icon_bag.close_image, [&](auto&) {
    ...
    m_copy_merged_action = GUI::Action::create(
        "Copy &Merged", { Mod_Ctrl | Mod_Shift, 'C' }, g_icon_bag.edit_copy, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_edit_menu->add_separator();
    m_edit_menu->add_action(GUI::Action::create(
        "S&wap Colors", { Mod_None, 'X' }, g_icon_bag.swap_colors, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_edit_menu->add_action(GUI::Action::create(
        "&Default Colors", { Mod_None, 'D' }, g_icon_bag.default_colors, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_show_rulers_action = GUI::Action::create_checkable(
        "Show R&ulers", { Mod_Ctrl, 'R' }, [&](auto& action) {
    ...
    m_image_menu->add_action(GUI::Action::create("Rotate Image &Counterclockwise", { Mod_Ctrl, '<' }, { Mod_Ctrl | Mod_Shift, '<' }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/edit-rotate-ccw.png"sv)),
        [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_image_menu->add_action(GUI::Action::create("Rotate Image Clock&wise", { Mod_Ctrl, '>' }, { Mod_Ctrl | Mod_Shift, '>' }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/edit-rotate-cw.png"sv)),
        [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_image_menu->add_separator();
    m_image_menu->add_action(GUI::Action::create(
        "&Resize Image...", { Mod_Ctrl | Mod_Shift, 'R' }, g_icon_bag.resize_image, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_levels_dialog_action = GUI::Action::create(
        "Change &Levels...", { Mod_Ctrl, 'L' }, g_icon_bag.levels, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_layer_menu->add_action(GUI::Action::create(
        "New &Layer...", { Mod_Ctrl | Mod_Shift, 'N' }, g_icon_bag.new_layer, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_layer_menu->add_action(GUI::Action::create(
        "Duplicate Layer", { Mod_Ctrl | Mod_Shift, 'D' }, g_icon_bag.new_layer, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_layer_via_copy = GUI::Action::create(
        "Layer via Copy", { Mod_Ctrl | Mod_Shift, 'C' }, g_icon_bag.new_layer, [&](auto&) {
    ...
    m_layer_via_cut = GUI::Action::create(
        "Layer via Cut", { Mod_Ctrl | Mod_Shift, 'X' }, g_icon_bag.new_layer, [&](auto&) {
            auto add_layer_success = current_image_editor()->add_new_layer_from_selection();
            if (add_layer_success.is_error()) {
                GUI::MessageBox::show_error(&window, MUST(String::formatted("{}", add_layer_success.release_error())));
                return;
    ...
    m_add_mask_action = GUI::Action::create(
        "Add M&ask", { Mod_Ctrl | Mod_Shift, 'M' }, g_icon_bag.add_mask, create_layer_mask_callback("Add Mask", [&](Layer* active_layer) {
    ...
    m_add_editing_mask_action = GUI::Action::create(
        "Add E&diting Mask", { Mod_Ctrl | Mod_Alt, 'E' }, g_icon_bag.add_mask, create_layer_mask_callback("Add Editing Mask", [&](Layer* active_layer) {
            VERIFY(!active_layer->is_masked());
    ...
    m_layer_menu->add_action(GUI::Action::create(
        "Select &Previous Layer", { Key_PageUp }, g_icon_bag.previous_layer, [&](auto&) {
            m_layer_list_widget->cycle_through_selection(1);
        }));
    m_layer_menu->add_action(GUI::Action::create(
        "Select &Next Layer", { Key_PageDown }, g_icon_bag.next_layer, [&](auto&) {
            m_layer_list_widget->cycle_through_selection(-1);
        }));
    m_layer_menu->add_action(GUI::Action::create(
        "Select &Top Layer", { Key_Home }, g_icon_bag.top_layer, [&](auto&) {
            m_layer_list_widget->select_top_layer();
        }));
    m_layer_menu->add_action(GUI::Action::create(
        "Select B&ottom Layer", { Key_End }, g_icon_bag.bottom_layer, [&](auto&) {
    ...
    m_layer_menu->add_separator();
    m_layer_menu->add_action(GUI::Action::create(
        "&Remove Active Layer", { Mod_Ctrl, 'D' }, g_icon_bag.delete_layer, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
            auto active_layer = editor->active_layer();
            if (!active_layer)
    ...
    m_layer_menu->add_separator();
    m_layer_menu->add_action(GUI::Action::create(
        "Fl&atten Image", { Mod_Ctrl, 'F' }, g_icon_bag.flatten_image, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_layer_menu->add_action(GUI::Action::create(
        "&Merge Visible", { Mod_Ctrl, 'M' }, g_icon_bag.merge_visible, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
    ...
    m_layer_menu->add_action(GUI::Action::create(
        "M&erge Active Layer Down", { Mod_Ctrl, 'E' }, g_icon_bag.merge_active_layer_down, [&](auto&) {
            auto* editor = current_image_editor();
            VERIFY(editor);
            auto active_layer = editor->active_layer();
            if (!active_layer)
