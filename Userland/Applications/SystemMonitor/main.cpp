...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto kill_action = GUI::Action::create(
        "&Kill Process", { Mod_Ctrl, 'K' }, { Key_Delete }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/kill.png"sv)), [&](const GUI::Action&) {
    ...
    auto stop_action = GUI::Action::create(
        "&Stop Process", { Mod_Ctrl, 'S' }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/stop-hand.png"sv)), [&](const GUI::Action&) {
            pid_t pid = selected_id(ProcessModel::Column::PID);
            if (pid == -1)
                return;
    ...
    auto continue_action = GUI::Action::create(
        "&Continue Process", { Mod_Ctrl, 'C' }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/continue.png"sv)), [&](const GUI::Action&) {
            pid_t pid = selected_id(ProcessModel::Column::PID);
            if (pid != -1)
                kill(pid, SIGCONT);
        },
    ...
    auto debug_action = GUI::Action::create(
        "Debug in HackStudio", { Mod_Ctrl, 'D' }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/app-hack-studio.png"sv)), [&](const GUI::Action&) {
            pid_t pid = selected_id(ProcessModel::Column::PID);
            if (pid == -1)
                return;
            auto pid_string = ByteString::number(pid);
    ...
    auto refresh_action = GUI::Action::create("&Refresh", { Mod_Ctrl, 'R' }, Key_F5, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/reload.png"sv)), [&refresh_timer, &update_stats](auto&) {
