...
ComboBox::ComboBox()
{
    ...
    m_editor->on_keypress = [this](KeyEvent& event) {
        if (!m_only_allow_values_from_model)
            return;
        if (!m_list_window->is_visible() && event.key_code() <= KeyCode::Key_Z && event.key_code() >= KeyCode::Key_A && event.modifiers() == Mod_None) {
            open();
    ...
    m_open_button->set_icon(MUST(Gfx::Bitmap::load_from_uri("resource://icons/16x16/downward-triangle.png"sv)));
